...
class Template(object):
	...
	def procesar_img(self, msg):
		...
		bridge=CvBridge()
		#se define una imagen de entrada y de salida
		image_in= bridge.imgmsg_to_cv2(msg, "bgr8") #imagen de entrada en BGR         
		image_out= cv2.cvtColor(image_in,cv2.COLOR_BGR2HSV) #imagen de salida en BGR2HSV.......
		#filtrar rango util
		lower_limit=np.array([50,100,100])
		upper_limit=np.array([90,255,255])
		#se crea una mascara con los colores indicados anteriormente
		mask=cv2.inRange(image_out,lower_limit,upper_limit)
		kernel= np.ones((5,5), np.uint8)
		#aplica erode y dilate a la mascara para precisar la imagen		
		mask = cv2.erode(mask,kernel, iterations = 1)
		mask = cv2.dilate(mask,kernel, iterations = 4)
		image_od=cv2.bitwise_and(image_in,image_in,mask=mask) #imagen a publicar 
